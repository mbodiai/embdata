...
def unflatten_from_schema(obj, schema, target_model: type[BaseModel] | None = None) -> dict:  # noqa: C901
    ...
    def unflatten_recursive(schema_part, index=0):
        if schema_part["type"] == "object":
            result = {} if schema_part.get("title") != "Sample" else target_model()
            for prop, prop_schema in schema_part["properties"].items():
                value, index = unflatten_recursive(prop_schema, index)
                value = target_model(**value) if prop_schema.get("title") == "Sample" else value
                result[prop] = value
            if schema_part.get("title") == "Sample":
                return target_model(**result), index
            return result, index
        if schema_part["type"] == "array":
            ...
            if schema_part.get("shape"):
                ...
                if all(isinstance(i, list | tuple | np.ndarray | float | int) for i in array):
                    ...
                    if schema_part.get("title") == "Sample":
                        return target_model(**items), index
                    return result, index
